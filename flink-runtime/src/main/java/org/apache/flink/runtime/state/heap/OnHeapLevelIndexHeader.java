...
public class OnHeapLevelIndexHeader implements LevelIndexHeader {
	...
	public OnHeapLevelIndexHeader(int maxLevel) {
		Preconditions.checkArgument(maxLevel >= 0 && maxLevel <= MAX_LEVEL,
	...
	@Override
	public void updateTopLevel(int level) {
		Preconditions.checkArgument(level >= 1 && level <= MAX_LEVEL,
	...
	@Override
	public void updateNextNode(int level, long newNodeId) {
		Preconditions.checkArgument(level >= 0 && level <= topLevel,
			"invalid level " + level + " current top level is " + topLevel);
